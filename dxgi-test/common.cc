...
int WritePng(png_bytep buffer, int width, int height, const char* file_name)
{
	...
	*/
	...
	// 设置IHDR
	png_set_IHDR(png_ptr, png_info_ptr, width, height, 8, PNG_COLOR_TYPE_RGBA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
	// 写入文件头部信息
	png_write_info(png_ptr, png_info_ptr);
	png_bytep change_buffer = new png_byte[width * height * 4];
	for (int i = 0; i < width * height; i++)
	{
		png_bytep src = buffer + i * 4;
		png_bytep dest = change_buffer + i * 4;
		dest[0] = src[2];
		dest[1] = src[1];
		dest[2] = src[0];
		dest[3] = src[3];
	...
	{
		row_pointers[k] = change_buffer + k * width * 4;
	}
	// 写入像素内容
